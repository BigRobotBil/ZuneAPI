...
public class GETOtherImage extends AbstractRoute {

    public GETOtherImage(Storage storage, JAXBContext jaxb) {
        super(storage, jaxb);
    }
    ...
    public byte[] handle(Map<ParamEnum, String> params) {
        ...
        try {
            ...
            logger.error("Failed to capture image information for '" + id + "' when fetching Other Image", e);
    ...
    private ByteArrayOutputStream fetchItem(String id) throws IOException {
        ...
        try (FileInputStream fileInStr = new FileInputStream("image/other/" + id + ".jpg")) {
